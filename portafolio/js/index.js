...
let isPowerOn = true;
        ...
        function triggerReset() {
            ...
            setTimeout(() => {
                ...
                // Re-disparar la animación de "encendido"
        ...
        // Geolocalización del visitante (ciudad por IP, con respaldos)
        function startHudGeo() {
            ...
            const setText = (txt) => { el.textContent = (txt || 'España').toUpperCase(); };
            const cached = sessionStorage.getItem('cj_geo');
            if (cached) { setText(cached); return; }
            setText(fromTimezone());
            fetch('https://ipapi.co/json/')
                ...
                .then(d => {
                    ...
                    const label = parts.join(' · ') || fromTimezone();
                    sessionStorage.setItem('cj_geo', label);
                    setText(label);
                })
                .catch(() => {});
        }
        ...
        window.onload = () => {
            ...
            startHudClock();
            startHudGeo();
            // Entrada inmediata: el contenido ya está en el DOM (bueno para SEO);
            // solo animamos su aparición tipo "encendido".
            requestAnimationFrame(() => document.body.classList.add('booted'));
            setTimeout(() => { fillTechBars(); playLogoAnim(); }, 600);
        };
/* ==========================================
